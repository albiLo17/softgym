...
from softgym.action_space.action_space import Dragger, Picker, PickerPickPlace, PickerQPG
from softgym.action_space.robot_env import RobotBase
from copy import deepcopy
from softgym.utils.misc import quatFromAxisAngle


class ClothBoxEnv(FlexEnv):
    ...
    def set_scene(self, config, state=None):
        ...
        center = np.array([0.15, 0.3, 0.])
        radius = 0.07
        pyflex.add_sphere(radius, center, [1, 0, 0, 0])

        # self.box_states = pyflex.get_shape_states().reshape(-1,14)[2,:]
        # self.box_states[:3] = np.array([-0.1, -0.1, -0.1])
        # states = pyflex.get_shape_states().reshape(-1,14)
        # states[2, :3] = self.box_states[:3]
